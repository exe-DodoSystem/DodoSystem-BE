...
> Cập nhật: 2026-06-02 (phản ánh toàn bộ trạng thái sau khi hoàn thành payroll_fix_checklist.md)
...
1. **Generate Phase** — Admin gọi API → hệ thống đọc `DailyTimesheet` của tháng, tính toán và tạo bản nháp `Payroll` (Draft)
2. **Review Phase** — Admin/HRManager xem, chỉnh sửa `CustomBonus` / `CustomDeduction` thủ công
3. **Publish Phase** — Admin chốt phiếu lương (`Published`) — nhân viên có thể xem
4. **Paid Phase** — Admin đánh dấu đã thanh toán (`Paid`) — hoàn tất vòng đời
...
| `PenaltyFee` | `decimal` | Phạt đi trễ/về sớm theo phút |
| `CustomBonus` | `decimal?` | Thưởng thủ công (Admin nhập) |
| `CustomDeduction` | `decimal` | Khấu trừ khác (Admin nhập — thuế, BHXH nếu có) |
| `NetSalary` | `decimal` | Lương thực nhận = `BasePay + OTPay - PenaltyFee + CustomBonus - CustomDeduction` |
...
| `PUT` | `/{payrollId}/manual-fields` | TenantAdmin, HRManager, Manager | Chỉnh `CustomBonus` / `CustomDeduction` (chỉ khi Draft) |
...
| DTO | Dùng cho |
|-----|---------|
| `PayrollDto` | Response của mọi endpoint trả phiếu lương — gồm đầy đủ thông tin nhân viên + các số liệu lương |
...
Xử lý toàn bộ nghiệp vụ Payroll: tính lương, chuyển trạng thái, query, chỉnh sửa thủ công.
...
| `IMapper` | Map `Payroll` → `PayrollDto` |
...
// Tính basePay, otPay, penaltyFee, netSalary
...
Tháng 5/2026: standardDays = 20 (tháng 5 có 1 ngày lễ 1/5)
actualDays = 19 (nghỉ 1 ngày không phép)
lateMinutes = 30, earlyLeaveMinutes = 0
otHours = 4

dailyRate   = 10.000.000 / 20 = 500.000 VNĐ/ngày
hourlyRate  = 500.000 / 8     = 62.500 VNĐ/giờ
minuteRate  = 62.500 / 60     ≈ 1.042 VNĐ/phút

BasePay    = 500.000 × 19  = 9.500.000 VNĐ
OTPay      = 4 × 62.500 × 1.5 = 375.000 VNĐ
PenaltyFee = 30 × 1.042    ≈ 31.250 VNĐ

NetSalary  = 9.500.000 + 375.000 - 31.250 = 9.843.750 VNĐ
```
...
### Kịch bản 1: Chu kỳ lương chuẩn hàng tháng

```
[ADMIN — đầu tháng sau]

  [1] POST /api/payrolls/generate?month=5&year=2026
      ← Tạo Draft cho toàn bộ nhân viên active

  [2] GET /api/payrolls/paged?month=5&year=2026&status=Draft
      ← Admin xem danh sách, review số liệu

  [3] PUT /api/payrolls/{payrollId}/manual-fields   (lặp cho NV cần điều chỉnh)
      { "customBonus": 500000, "reason": "Thưởng KPI" }
      ← Chỉnh thủ công nếu cần

  [4] PUT /api/payrolls/publish-all?month=5&year=2026
      ← Chốt toàn bộ Draft → Published

[NHÂN VIÊN]
  [5] GET /api/payrolls/my?month=5&year=2026
      ← Thấy phiếu lương Status=Published
...
[NHÂN VIÊN]
  [7] GET /api/payrolls/my
      ← Thấy phiếu lương Status=Paid
```
...
[HR phát hiện nhân viên A bị tính sai do Attendance có lỗi]
  ...
  [1] (HR sửa dữ liệu Attendance bằng recalculate)
      POST /api/v1/attendance/recalculate/{employeeId}?from=2026-05-01&to=2026-05-31

  [2] POST /api/payrolls/calculate/{employeeId}?month=5&year=2026
      ← Tính lại, cập nhật bản Draft hiện có (giữ CustomBonus/Deduction cũ)

  [3] GET /api/payrolls/paged?employeeId={id}&month=5&year=2026
      ← Verify kết quả
...
[ADMIN — cuối tháng 5]
  ...
  [3] Tiếp tục chu kỳ bình thường (Kịch bản 1 bước 2–7)
```
...
2. **Idempotent generate** — gọi `generate` bao nhiêu lần cũng không gây hại: phiếu `Published/Paid` được bỏ qua, phiếu `Draft` được cập nhật với số liệu mới nhất
...
4. **MissingOut không bị phạt penalty** — ngày quên check-out được tính vào `actualDays` (nhân viên vẫn được tính lương ngày đó) nhưng không bị trừ phút về sớm (vì `AttendanceResolutionService` ghi `earlyLeaveMinutes` ≈ cả ca cho ngày MissingOut). HR cần xử lý thủ công qua `CustomDeduction` nếu muốn phạt
...
7. **NetSalary có thể âm về lý thuyết** — nếu `PenaltyFee + CustomDeduction > BasePay + OTPay + CustomBonus`. Hệ thống không có floor = 0, Admin cần kiểm tra thủ công

8. **Không có thuế TNCN / BHXH / BHYT tự động** — `CustomDeduction` là nơi Admin nhập thủ công các khoản này nếu cần. MVP chưa tính tự động
